...
let currentRow = 0;
let currentColumn = 0;
let letreco = "LETRA"; // Palavra a ser adivinhada
let letrecoMap = {};
...
const checkGuess = () => {
  ...
  if(guess === letreco) {
      window.alert("Então vc é o tal do xereque romes, simplesmente o detetivao do entreterimento!")
      return
  } {
...
const restartGame = () => {
    ...
    for (let i = 0; i < rows; i++) {
        guesses[i] = new Array(columns).fill("");
    }
    const allTiles = document.querySelectorAll('.tile-column');
    allTiles.forEach(tile => {
        tile.textContent = "";
        tile.className = "tile-column disabled";
    });

    const firstRowTiles = document.querySelectorAll('#row0 .tile-column');
    firstRowTiles.forEach(tile => {
        tile.classList.remove("disabled");
        tile.classList.add("typing");
    });
    
    document.querySelectorAll('.wrong, .right, .displaced').forEach(tile => {
        tile.classList.remove('wrong', 'right', 'displaced');
    });
};
